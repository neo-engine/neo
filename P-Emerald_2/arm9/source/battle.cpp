...
#include "dex.h"
    ...
    namespace BATTLE {
        ...
        battle::battle( pokemon* p_playerTeam, u16 p_opponentId, u16 p_maxRounds,
                        weather p_weather, u8 p_platform, u8 p_background, u16 p_AILevel,
                        battleMode p_battleMode, u8 p_platform2, bool p_allowMegaEvolution ) {
            _playerTeam         = p_playerTeam;
            _allowMegaEvolution = p_allowMegaEvolution;
            for( _playerTeamSize = 0; _playerTeamSize < 6; ++_playerTeamSize ) {
                if( !p_playerTeam[ _playerTeamSize ].m_boxdata.m_speciesId ) {
                    break;
                }
            }
        ...
        battle::battle( pokemon* p_playerTeam, pokemon p_opponent,
                        weather p_weather, u8 p_platform, u8 p_platform2, u8 p_background,
                        bool p_allowMegaEvolution ) {
            _maxRounds          = 0;
            _AILevel            = 0;
            _playerTeam         = p_playerTeam;
            _allowMegaEvolution = p_allowMegaEvolution;
     ...
     */
    s8 battle::start( ) {
        if( !_opponentTeamSize || !_playerTeamSize ) {
            return 2;
        }
        ...
        while( !_maxRounds || ++_round < _maxRounds ) {
            ...
            loop( ) {
                ...
                if( m_battleMode == DOUBLE ) {
                    ...
                    moves[ field::PLAYER_SIDE ][ 1 ] = getMoveSelection( 1, _allowMegaEvolution &&
            ...
            for( u8 i = 0; i < 2; ++i ) for( u8 j = 0; j < u8( m_battleMode ); ++j ) {
            ...
            for( size_t i = 0; i < sortedMoves.size( ); ++i ) {
                ...
                if( sortedMoves[ i ].m_type == battleMoveType::MEGA_ATTACK
                    ...
                    for( auto m : sortedMoves ) {
                        for( auto t : sortedMoves[ i ].m_target ) if( t == m.m_user ) {
                            targets.push_back( m );
                        }
                        for( auto t : m.m_target ) if( t == sortedMoves[ i ].m_user ) {
                            targetedBy.push_back( m );
                        }
                    }
