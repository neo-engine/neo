...
namespace BATTLE {
    ...
    bool field::useMove( battleUI* p_ui, battleMove p_move ) {
        ...
        if( hasStatusCondition( opponent, slot, FROZEN ) )
            [[unlikely]] {
                if( p_move.m_moveData.m_type == FIRE
                    ...
                    snprintf( buffer, 99, GET_STRING( 297 ),
                              p_ui->getPkmnName( getPkmn( opponent, slot ), opponent ).c_str( ) );
                    p_ui->log( buffer );
                    return false;
                }
        ...
        if( hasStatusCondition( opponent, slot, PARALYSIS ) ) {
            if( rand( ) % 100 < 25 ) {
                ...
                snprintf( buffer, 99, GET_STRING( 301 ),
                          p_ui->getPkmnName( getPkmn( opponent, slot ), opponent ).c_str( ) );
                p_ui->log( buffer );
                return false;
            }
        ...
        if( volst & CONFUSION )
            ...
            if( ( volst & HEALBLOCK )
                && ( p_move.m_moveData.m_flags & MOVE::HEAL ) )[ [unlikely] ] {
                return false;
            }
    ...
    bool field::executeCriticalCheck( battleUI* p_ui, battleMove p_move, fieldPosition p_target ) {
        ...
        if( item && user->getItem( ) == I_LANSAT_BERRY )
            [[unlikely]] {
                u8 factor = 4;
        ...
        if( critLevel >= 3 )
            [[unlikely]] { critical = true; } else if( critLevel == 2 )[ [unlikely] ] {
                critical = !( rand( ) % 2 );
            }
        else if( critLevel == 1 ) {
            critical = !( rand( ) % 8 );
        } else
            [[likely]] { critical = !( rand( ) % 24 ); }
    ...
    u16 field::getEffectiveness( battleMove p_move, fieldPosition p_target ) {
        ...
        if( target == nullptr )
            ...
            bool items
                = canUseItem( p_target.first, p_target.second );
        ...
        if( p_move.m_param == M_THOUSAND_ARROWS )
            ...
            for( type t : getTypes( p_target.first, p_target.second ) ) {
                ...
                if( !isGrounded( p_target.first, p_target.second,
                                 p_move.m_param != M_SUNSTEEL_STRIKE )
                    && moveType == GROUND )
                    [[unlikely]] { curval = 0; }
                ...
                if( t == GHOST && ( moveType == NORMAL || moveType == FIGHT ) )
                    [[unlikely]] {
                        if( !suppressesAbilities( )
                    ...
                    if( t == DARK && moveType == PSYCHIC
                        && ( getVolatileStatus( p_target.first, p_target.second )
                             & MIRACLEEYE ) )[ [unlikely] ] {
                        continue;
                    }
                ...
                if( t == FLYING ) {
                    if( !suppressesWeather( ) && _weather == HEAVY_WINDS )
                        [[unlikely]] { continue; }
                }

                if( !curval && items && target->getItem( ) == I_RING_TARGET )
                    [[unlikely]] { continue; }
    ...
    bool field::executeDamagingMove( battleUI* p_ui, battleMove p_move, fieldPosition p_target,
        ...
        if( p_move.m_moveData.m_fixedDamage ) {
            ...
            if( p_move.m_param == M_BODY_SLAM || p_move.m_param == M_DRAGON_RUSH
                ...
                if( getVolatileStatus( p_target.first, p_target.second ) & MINIMIZE )
                    [[unlikely]] { damage <<= 1; }
            }
    ...
    void field::executeBattleMove( battleUI* p_ui, battleMove p_move,
                ...
                if( ( p_move.m_param == M_SOLAR_BLADE || p_move.m_param == M_SOLAR_BEAM )
                    ...
                    [[unlikely]] {
                        // empty!
                               ...
                               && getPkmn( opponent, slot )->getItem( )
                                      == I_POWER_HERB )[ [unlikely] ] {
                        snprintf(
        ...
        for( u8 i = 0; i < p_move.m_target.size( ); ++i ) {
            ...
            if( p_move.m_moveData.getMultiHitMax( ) > 1 )
                [[unlikely]] {
                    ...
                    if( !suppressesAbilities( )
                        ...
                        [[unlikely]] { numHits = p_move.m_moveData.getMultiHitMax( ); }
            ...
            for( u8 j = 0; j < numHits; ++j ) {
                ...
                if( p_move.m_moveData.m_flags & MOVE::PROTECT )
                    ...
                    if( !protect && p_move.m_moveData.m_category != MOVE::STATUS )[ [likely] ] {
                        ...
                        if( !executeDamagingMove( p_ui, p_move, p_move.m_target[ i ], critical,
                                                  j == 1 && strengthMod < 100 ? strengthMod
                                                                              : 100 ) ) {
                            break;
                        }
                        hits++;
                ...
                if( !protect )
                    ...
                    // Pkmn fainted
                    if( getSlotStatus( opponent, slot ) == slot::status::FAINTED ) {
                        break;
                    }
                // Target fainted
                if( getSlotStatus( p_move.m_target[ i ].first, p_move.m_target[ i ].second )
                        ...
                        if( suppressesAbilities( )
                            ...
                            [[likely]] {
                                if( canUseItem( opponent, slot )
                                    ...
                                    addVolatileStatus( opponent, slot, CONFUSION,
                                                       2 + ( rand( ) % 4 ) );
                                }
                            } else {
        ...
        if( p_move.m_param == M_BURN_UP ) { addVolatileStatus( opponent, slot, BURNUP, -1 ); }
