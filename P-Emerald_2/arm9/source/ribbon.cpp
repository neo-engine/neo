...
std::vector<u8> ribbon::getRibbons( const pokemon& p_pokemon ) {
    std::vector<u8> res;
    u8              curr = 0;
    for( u8 j : {p_pokemon.m_boxdata.m_ribbons2[ 0 ], p_pokemon.m_boxdata.m_ribbons2[ 1 ],
                 p_pokemon.m_boxdata.m_ribbons2[ 2 ], p_pokemon.m_boxdata.m_ribbons2[ 3 ],

                 p_pokemon.m_boxdata.m_ribbons0[ 0 ], p_pokemon.m_boxdata.m_ribbons0[ 1 ],
                 p_pokemon.m_boxdata.m_ribbons0[ 2 ], p_pokemon.m_boxdata.m_ribbons0[ 3 ],

                 p_pokemon.m_boxdata.m_ribbons1[ 0 ], p_pokemon.m_boxdata.m_ribbons1[ 1 ],
                 p_pokemon.m_boxdata.m_ribbons1[ 2 ], p_pokemon.m_boxdata.m_ribbons1[ 3 ]} )
        for( u8 i = 0; i < 8; ++i, ++curr )
            if( j & ( 1 << i ) ) res.push_back( curr );
    return res;
}
