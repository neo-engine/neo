...
FILE	*ndsFile;
u32 ndsFileLastpos;	//Used to determine need to fseek or not
...
inline int nitroSubRead( u32 *npos, void *ptr, int len ) {
...
//seek around
inline void nitroSubSeek( u32 *npos, int pos, int dir ) {
...
int nitroFSInit( const char *ndsfile ) {
    ...
    ndsFileLastpos = 0;
    //First try opening via ndsfile
    if( ( ndsfile != NULL ) && ( ndsFile = fopen( ndsfile, "rb" ) ) ) {
        ...
        //we dont need double buffs u_u
        setvbuf( ndsFile, NULL, _IONBF, 0 );
        AddDevice( &nitroFSdevoptab );
        return( 1 );
    }
    ndsFile = NULL;
    //give us gba slot ownership
