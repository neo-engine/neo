...
#define PLAYERSPRITE 0
    ...
    inline void readTileSet( FILE* p_file, TileSet& p_tileSet, u16 p_startIdx = 0, u16 p_size = 512 ) {
        if( p_file == 0 )
            return;
        fread( &p_tileSet.m_blocks[ p_startIdx ], sizeof( Tile )*p_size, 1, p_file );
        fclose( p_file );
    }
    inline void readNop( FILE* p_file, u16 p_cnt ) {
        if( p_file == 0 )
            return;
        fread( 0, sizeof( u8 )*p_cnt, 1, p_file );
    }
    inline void readBlockSet( FILE* p_file, BlockSet& p_tileSet, u16 p_startIdx = 0, u16 p_size = 512 ) {
        if( p_file == 0 )
            return;
        readNop( p_file, 4 );
    ...
    Map::Map( const char* p_path, const char* p_name ) {
        ...
        if( A ) {
            ...
            for( u16 i = 0; i < N; ++i ) {
                ...
                for( u16 x = 0; x < ac.m_mapsx; ++x )
                    for( u16 y = 0; y < ac.m_mapsy; ++y ) {
    ...
    void Map::fill( u16* p_mapMemory[ 4 ], u16 p_xmin, u16 p_x, u16 p_xmax, u16 p_ymin, u16 p_y, u16 p_ymax, u16 p_c ) {
        u16 toplayer = 1, betw = 2, bottomlayer = 3;

        Block acBlock;
        if( p_x < 0 || p_y < 0 || p_x >= m_sizex + 20 || p_y >= m_sizey + 20 )
            acBlock = m_blockSets.m_blocks[ m_rand[ p_x % 2 ][ p_y % 2 ] ];
        else if( ( p_x < 10 || p_y < 10 || p_x >= m_sizex + 10 || p_y >= m_sizey + 10 ) && m_blocks[ p_x ][ p_y ].m_blockidx == 0 )
            acBlock = m_blockSets.m_blocks[ m_rand[ p_x % 2 ][ p_y % 2 ] ];
        else
    ...
    void Map::movePlayer( u16 p_direction ) {
        u16 c = 0;
        u16* mapMemory[ 4 ];
        for( u8 i = 1; i < 4; ++i )  mapMemory[ i ] = (u16*)BG_MAP_RAM( 2 * i - 1 );

        u16 bx = lastbx, by = lastby;
    ...
    void Map::draw( u16 p_bx, u16 p_by, bool p_init ) {
        if( p_init ) {
        ...
        lastby = p_by;
        for( u8 i = 0; i < 2; ++i ) {
            for( u16 x = p_by; x < p_by + 17; x++ ) {
                for( u16 y = p_bx; y < p_bx + 17; y++ ) {
                    u16 toplayer = 1, bottomlayer = 3;
                    ...
                    //    tcnt = (tcnt +1)%1024;
                    //}
                    if( x < 0 || y < 0 || x >= m_sizex + 20 || y >= m_sizey + 20 ) {
                        acBlock = m_blockSets.m_blocks[ m_rand[ x % 2 ][ y % 2 ] ];
                    } else if( ( x < 10 || y < 10 || x >= m_sizex + 10 || y >= m_sizey + 10 ) && m_blocks[ x ][ y ].m_blockidx == 0 ) {
                        acBlock = m_blockSets.m_blocks[ m_rand[ x % 2 ][ y % 2 ] ];
                    } else {
                        //consoleSelect(&Top);
