...
namespace BATTLE {
    ...
    bool battleScript::command::condition::check( battle& p_battle, void* p_self ) const {
        ...
        case BATTLE::battleScript::command::OPPONENT1:
            return evaluate(
                p_battle.getTargetSpecifierValue( OPPONENT, 0, m_targetSpecifier, m_value ) );
        case BATTLE::battleScript::command::OPPONENT2:
            return evaluate(
                p_battle.getTargetSpecifierValue( OPPONENT, 1, m_targetSpecifier, m_value ) );
        case BATTLE::battleScript::command::OWN1:
            return evaluate(
                p_battle.getTargetSpecifierValue( PLAYER, 0, m_targetSpecifier, m_value ) );
        case BATTLE::battleScript::command::OWN2:
            return evaluate(
                p_battle.getTargetSpecifierValue( PLAYER, 1, m_targetSpecifier, m_value ) );
        ...
        case BATTLE::battleScript::command::SELF_PKMN: {
            ...
            return evaluate( p_battle.getTargetSpecifierValue( pkmnIsOpp, pkmnIsSnd,
                                                               m_targetSpecifier, m_value ) );
        }
    ...
    int battleScript::command::value::get( battle& p_battle, bool p_targetIsOpp,
                                           u8 p_targetPosition ) const {
        return m_additiveConstant
               + int( m_multiplier
                      * p_battle.getTargetSpecifierValue( p_targetIsOpp, p_targetPosition,
                                                          m_targetSpecifier ) );
    }
    int battleScript::command::value::get( battle& p_target ) const {
        return m_additiveConstant
               + int( m_multiplier * p_target.getTargetSpecifierValue( m_targetSpecifier ) );
    }

    void battleScript::command::evaluateOnTargetVal( battle& p_battle, void* p_self,
                                                     bool p_targetIsOpp,
                                                     u8   p_targetPosition ) const {
        if( p_battle.m_battleMode != battle::DOUBLE && p_targetPosition ) return;
        ...
        case BATTLE::battleScript::command::SET:
            ...
            case BATTLE::battleScript::command::PKMN_HP: {
                ...
                if( m_action == SET ) newVal = m_value.get( p_battle, p_self );
                if( m_action == ADD )
                    newVal = (s16) tgt.m_pokemon->m_stats.m_acHP + m_value.get( p_battle, p_self );
                if( m_action == MULTIPLY )
                    newVal = int( ( tgt.m_pokemon->m_stats.m_acHP / 100.f )
                                  * m_value.get( p_battle, p_self ) );

                tgt.m_pokemon->m_stats.m_acHP
                    = std::max( (s16) 0, std::min( (s16) tgt.m_pokemon->m_stats.m_maxHP, newVal ) );

                break;
            }
            case BATTLE::battleScript::command::PKMN_ATK:
                if( m_action == SET ) tgt.m_acStatChanges[ ATK ] = m_value.get( p_battle, p_self );
                if( m_action == ADD ) tgt.m_acStatChanges[ ATK ] += m_value.get( p_battle, p_self );
                break;
            case BATTLE::battleScript::command::PKMN_DEF:
                if( m_action == SET ) tgt.m_acStatChanges[ DEF ] = m_value.get( p_battle, p_self );
                if( m_action == ADD ) tgt.m_acStatChanges[ DEF ] += m_value.get( p_battle, p_self );
                break;
            case BATTLE::battleScript::command::PKMN_SPD:
                if( m_action == SET ) tgt.m_acStatChanges[ SPD ] = m_value.get( p_battle, p_self );
                if( m_action == ADD ) tgt.m_acStatChanges[ SPD ] += m_value.get( p_battle, p_self );
                break;
            case BATTLE::battleScript::command::PKMN_SATK:
                if( m_action == SET ) tgt.m_acStatChanges[ SATK ] = m_value.get( p_battle, p_self );
                if( m_action == ADD )
                    tgt.m_acStatChanges[ SATK ] += m_value.get( p_battle, p_self );
                break;
            case BATTLE::battleScript::command::PKMN_SDEF:
                if( m_action == SET ) tgt.m_acStatChanges[ SDEF ] = m_value.get( p_battle, p_self );
                if( m_action == ADD )
    ...
    void battleScript::command::execute( battle& p_battle, void* p_self ) const {
        ...
        case BATTLE::battleScript::command::MOVE: {
            ...
            case BATTLE::battleScript::command::MULTIPLY:
                *( (int*) p_self )
                    = int( *( (int*) p_self ) * m_value.get( p_battle, p_self ) / 100.f );
                break;
            default:
                break;
            }
    ...
    void battleScript::execute( battle& p_battle, void* p_self ) const {
        ...
        for( auto c : _commands ) {
            for( auto cond : c.m_conditions ) {
                if( cond.m_asLastCondition && !lastCondition ) goto NEXT;
