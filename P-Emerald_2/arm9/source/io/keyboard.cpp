...
namespace IO {
    ...
    keyboard::keyboard( ) {
        ...
        std::wstring spchars[ ] = { L"ÄÖÜ .!?()[]{}", L"äöüßé §$%&/\\#" };
    ...
    std::wstring keyboard::getText( u16 p_length, const char* p_msg, bool p_time, bool p_timeParameter ) {
        ...
        while( 1 ) {
            ...
            u16 keyPosX = ( t.px - x ) / ( width + margin );
            u16 keyPosY = ( t.py - y ) / ( height + margin );
            ...
            if( i < p_length && _chars.count( { _ind, { nx, ny } } ) ) {
                ...
                swiWaitForVBlank( );
                       ...
                       && waitForTouchUp( p_time, p_timeParameter, 232, 146, 264, 172 ) ) {
                _ind = ( _ind + 1 ) % MAXKEYBOARDS;
                draw( p_msg, out, p_length );
                if( p_time )
                    updateTime( p_timeParameter );
            } else if( sqrt( sq( 248 - t.px ) + sq( 184 - t.py ) ) <= 16
                       && waitForTouchUp( p_time, p_timeParameter, 232, 168, 264, 200 ) ) {
                return out;
            } else if( ( sqrt( sq( 220 - t.px ) + sq( 184 - t.py ) ) <= 16 )
                       && waitForTouchUp( p_time, p_timeParameter, 204, 168, 236, 200 ) ) {
                out.pop_back( );
                --i;
