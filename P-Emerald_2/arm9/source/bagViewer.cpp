...
#define CURRENT_ITEM SAVE::SAV->getActiveFile( ).m_bag( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, \
    ...
    bool bagViewer::useItemOnPkmn( pokemon& p_pokemon, u16 p_item ) {
        ...
        if( ItemList[ p_item ]->m_itemType == item::itemType::MEDICINE ) {
            ...
            if( ItemList[ p_item ]->use( p_pokemon ) ) {
                ...
                SAVE::SAV->getActiveFile( ).m_bag.erase( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, p_item, 1 );
    ...
    */
    ...
    bool bagViewer::handleSomeInput( touchPosition p_touch, int p_pressed ) {
        ...
        auto curBgsz = SAVE::SAV->getActiveFile( ).m_bag.size( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag );
        ...
        if( GET_AND_WAIT( KEY_B ) || GET_AND_WAIT( KEY_X ) || ( !_hasSprite &&  GET_AND_WAIT_C( SCREEN_WIDTH - 12, SCREEN_HEIGHT - 10, 16 ) ) ) {
            ...
            _currSelectedIdx = 0;
            SAVE::SAV->getActiveFile( ).m_lstBag = ( SAVE::SAV->getActiveFile( ).m_lstBag + BAG_CNT - 1 ) % BAG_CNT;
            if( SAVE::SAV->getActiveFile( ).m_bag.size( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag ) )
                SAVE::SAV->getActiveFile( ).m_lstBagItem %= SAVE::SAV->getActiveFile( ).m_bag.size( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag );
            else
            ...
            _currSelectedIdx = 0;
            SAVE::SAV->getActiveFile( ).m_lstBag = ( SAVE::SAV->getActiveFile( ).m_lstBag + 1 ) % BAG_CNT;
            if( SAVE::SAV->getActiveFile( ).m_bag.size( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag ) )
                SAVE::SAV->getActiveFile( ).m_lstBagItem %= SAVE::SAV->getActiveFile( ).m_bag.size( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag );
            else
                SAVE::SAV->getActiveFile( ).m_lstBagItem = 0;
            ...
            _bagUI->unselectItem( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, _currSelectedIdx, CURRENT_ITEM.first );
            u8 mx = std::min( 9u, curBgsz );
            if( ++_currSelectedIdx == mx ) {
                _currSelectedIdx = 0;
                SAVE::SAV->getActiveFile( ).m_lstBagItem = ( SAVE::SAV->getActiveFile( ).m_lstBagItem + mx ) % curBgsz;
                _ranges = _bagUI->drawBagPage( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, SAVE::SAV->getActiveFile( ).m_lstBagItem );
            }
            _bagUI->selectItem( _currSelectedIdx, CURRENT_ITEM );
            ...
            if( _currSelectedIdx-- == 0 ) {
                ...
                SAVE::SAV->getActiveFile( ).m_lstBagItem = ( SAVE::SAV->getActiveFile( ).m_lstBagItem + curBgsz - mx ) % curBgsz;
                _ranges = _bagUI->drawBagPage( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, SAVE::SAV->getActiveFile( ).m_lstBagItem );
            }
            _bagUI->selectItem( _currSelectedIdx, CURRENT_ITEM );
        } else if( !_hasSprite && GET_AND_WAIT_C( SCREEN_WIDTH - 44, SCREEN_HEIGHT - 10, 16 ) ) {
            if( !curBgsz ) return true;
            SAVE::SAV->getActiveFile( ).m_lstBagItem = ( SAVE::SAV->getActiveFile( ).m_lstBagItem + 8 ) % curBgsz;
            _ranges = _bagUI->drawBagPage( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, SAVE::SAV->getActiveFile( ).m_lstBagItem );
            _bagUI->selectItem( _currSelectedIdx, CURRENT_ITEM );
        } else if( !_hasSprite && GET_AND_WAIT_C( SCREEN_WIDTH - 76, SCREEN_HEIGHT - 10, 16 ) ) {
            if( !curBgsz ) return true;
            SAVE::SAV->getActiveFile( ).m_lstBagItem = ( SAVE::SAV->getActiveFile( ).m_lstBagItem + curBgsz - 8 ) % curBgsz;
            _ranges = _bagUI->drawBagPage( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, SAVE::SAV->getActiveFile( ).m_lstBagItem );
            _bagUI->selectItem( _currSelectedIdx, CURRENT_ITEM );
        }

        for( u8 i = 0; i < 5; ++i )
            if( i != SAVE::SAV->getActiveFile( ).m_lstBag && !_hasSprite && GET_AND_WAIT_C( 26 * i + 13, 3 + 13, 13 ) ) {
            ...
            case 1: //Use
            {
                ...
                if( !itm->use( true ) )
                    ...
                    if( itm->m_itemType != item::KEY_ITEM )
                        SAVE::SAV->getActiveFile( ).m_bag.erase( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, CURRENT_ITEM.first, 1 );
                }
                break;
            }
            case 2: //Toss everything
                SAVE::SAV->getActiveFile( ).m_bag.erase( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, CURRENT_ITEM.first );
                break;
            default:
                ret = 0;
                break;
        ...
        loop( ) {
            ...
            if( _hasSprite && !( touch.px | touch.py ) ) { //Player drops the sprite at hand
                if( !_ranges[ start ].second.m_isHeld )
                    _bagUI->dropSprite( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, start, CURRENT_ITEM );
                else
                    _bagUI->dropSprite( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, MAX_ITEMS_PER_PAGE, { _ranges[ start ].second.m_item, 1 } );
                _hasSprite = false;
                ...
                if( curr != (u8) -1 ) {
                    if( _ranges[ start ].second.m_isHeld && _ranges[ curr ].second.m_isHeld ) { //Swap held items
                        if( !SAVE::SAV->getActiveFile( ).m_pkmnTeam[ start - t ].isEgg( )
                            ...
                            std::swap( SAVE::SAV->getActiveFile( ).m_pkmnTeam[ start - t ].m_boxdata.m_holdItem,
                                       SAVE::SAV->getActiveFile( ).m_pkmnTeam[ curr - t ].m_boxdata.m_holdItem );
                    } else if( !_ranges[ start ].second.m_isHeld && !_ranges[ curr ].second.m_isHeld ) { //Swap bag items
                        SAVE::SAV->getActiveFile( ).m_bag.swap( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, SAVE::SAV->getActiveFile( ).m_lstBagItem + start, SAVE::SAV->getActiveFile( ).m_lstBagItem + curr );
                        std::swap( _ranges[ start ].second.m_item, _ranges[ curr ].second.m_item );
                        _currSelectedIdx = curr;
                        _bagUI->unselectItem( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, start, _ranges[ start ].second.m_item );
                        _bagUI->selectItem( _currSelectedIdx, CURRENT_ITEM );
                        continue;
                        ...
                        if( result )
                            SAVE::SAV->getActiveFile( ).m_bag.erase( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, _ranges[ start ].second.m_item, 1 );
                    }
                            ...
                            if( ( !touch.px && !touch.py ) || c++ == TRESHOLD ) {
                                ...
                                if( !i.second.m_isHeld || i.second.m_item ) {
                                    if( SAVE::SAV->getActiveFile( ).m_bag.size( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag ) )
                                        ...
                                        _bagUI->unselectItem( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag, MAX_ITEMS_PER_PAGE, 0 );
                                }
                                if( !i.second.m_isHeld ) {
                                    ...
                                    if( j < MAX_ITEMS_PER_PAGE )
                                        vl += MAX_ITEMS_PER_PAGE - SAVE::SAV->getActiveFile( ).m_bag.size( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag );
    ...
    u16 bagViewer::getItem( context p_context ) {
        ...
        loop( ) {
            ...
            if( GET_AND_WAIT( KEY_A ) ) {
                if( SAVE::SAV->getActiveFile( ).m_bag.empty( ( bag::bagType )SAVE::SAV->getActiveFile( ).m_lstBag ) )
                    continue;
