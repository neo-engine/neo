...
pokemon::stats calcStats( const pokemon::boxPokemon& p_boxdata, u8 p_level, const pokemonData& p_data ) {
    ...
    if( pkmnId != 292 )
        res.m_acHP = res.m_maxHP = ( ( p_boxdata.m_individualValues.m_hp + 2 * p_data.m_bases[ 0 ] + ( p_boxdata.m_effortValues[ 0 ] / 4 ) + 100 )*p_level / 100 ) + 10;
    else
        res.m_acHP = res.m_maxHP = 1;
    pkmnNatures nature = p_boxdata.getNature( );
    res.m_Atk = ( ( ( p_boxdata.m_individualValues.m_attack + 2 * p_data.m_bases[ 1 ] + ( p_boxdata.m_effortValues[ 1 ] >> 2 ) )*p_level / 100.0 ) + 5 )*NatMod[ nature ][ 0 ];
    res.m_Def = ( ( ( p_boxdata.m_individualValues.m_defense + 2 * p_data.m_bases[ 2 ] + ( p_boxdata.m_effortValues[ 2 ] >> 2 ) )*p_level / 100.0 ) + 5 )*NatMod[ nature ][ 1 ];
    res.m_Spd = ( ( ( p_boxdata.m_individualValues.m_speed + 2 * p_data.m_bases[ 5 ] + ( p_boxdata.m_effortValues[ 5 ] >> 2 ) )*p_level / 100.0 ) + 5 )*NatMod[ nature ][ 2 ];
    res.m_SAtk = ( ( ( p_boxdata.m_individualValues.m_sAttack + 2 * p_data.m_bases[ 3 ] + ( p_boxdata.m_effortValues[ 3 ] >> 2 ) )*p_level / 100.0 ) + 5 )*NatMod[ nature ][ 3 ];
    res.m_SDef = ( ( ( p_boxdata.m_individualValues.m_sDefense + 2 * p_data.m_bases[ 4 ] + ( p_boxdata.m_effortValues[ 4 ] >> 2 ) )*p_level / 100.0 ) + 5 )*NatMod[ nature ][ 4 ];
    return res;
}
...
u16 calcLevel( const pokemon::boxPokemon& p_boxdata, const pokemonData& p_data ) {
    for( u16 i = 2; i < 101; ++i )
        if( EXP[ i - 1 ][ p_data.m_expType ] > p_boxdata.m_experienceGained )
            return ( i - 1 );
    return 100;
}
