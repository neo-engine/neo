...
#define RIBBON_DETAILS 3
        ...
        loop( ) {
            ...
            for( u8 i = 0; i < 5; ++i ) {
                if( SAVE::SAV->getActiveFile( ).m_pkmnTeam[ _pkmnIdx ].m_boxdata.m_individualValues.m_isEgg && i != 2 )
                    continue;
            ...
            for( s8 i = 0; i < s8( tg.size( ) ) - 2; ++i )
                if( IN_RANGE_I( touch, tg[ i ] ) && IO::waitForInput( tg[ i ] ) ) {
                    u8 u = 0, o;
                    for( o = 0; o < 4 && u <= i; ++o )
                        if( AttackList[ SAVE::SAV->getActiveFile( ).m_pkmnTeam[ _pkmnIdx ].m_boxdata.m_moves[ o ] ]->m_isFieldAttack )
                            u++;
                    o--;
            ...
            if( tg.size( ) >= 2 ) {
                if( IN_RANGE_I( touch, tg[ tg.size( ) - 2 ] ) && IO::waitForInput( tg[ tg.size( ) - 2 ] ) ) {
                    if( SAVE::SAV->getActiveFile( ).m_pkmnTeam[ _pkmnIdx ].m_boxdata.m_holdItem ) { //take item
                        char buffer[ 50 ];
                        item acI = *ItemList[ SAVE::SAV->getActiveFile( ).m_pkmnTeam[ _pkmnIdx ].m_boxdata.m_holdItem ];
                        ...
                        if( itm ) {
                            if( SAVE::SAV->getActiveFile( ).m_pkmnTeam[ _pkmnIdx ].getItem( ) ) {
                                auto curItm = ItemList[ SAVE::SAV->getActiveFile( ).m_pkmnTeam[ _pkmnIdx ].getItem( ) ];
                                SAVE::SAV->getActiveFile( ).m_bag.insert( BAG::toBagType( curItm->m_itemType ), curItm->getItemId( ), 1 );
                            }
