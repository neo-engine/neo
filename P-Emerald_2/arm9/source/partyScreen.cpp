...
namespace STS {
    ...
    bool partyScreen::confirmSelection( ) {
        ...
        IO::yesNoBox yn;
        bool res = yn.getResult( [&]( ) {
                    return _partyUI->printYNMessage( 0, 254 );
    ...
    void partyScreen::selectChoice( u8 p_choice, u8 p_numChoices ) {
        ...
        _ranges = _partyUI->drawPartyPkmnChoice( _currentSelection, 0,
        ...
        case DESQUID_SPECIES: {
            ...
            res.push_back( partyScreen::desquidItem(
                {DESQUID_STRING + 22, false, 1640000, 0,
                 ...
                 [&]( u32 p_newValue ) { _team[ _currentSelection ].setExperience( p_newValue ); },
                 []( u32 ) { return ""; }} ) );
            break;
        }
        case DESQUID_STATUS: {
...
#undef UPDATE_VALUE
    ...
    bool STS::partyScreen::desquid( u8 p_selectedIdx ) {
        ...
        _ranges = _partyUI->drawPartyPkmnChoice( p_selectedIdx, c, std::min( size_t( 6 ),
                                                choices.size( ) ), choices.size( ) > 6, false, 0 );
        ...
        loop( ) {
            ...
            if( GET_KEY_COOLDOWN( KEY_RIGHT ) ) {
                ...
                if( ( _currentChoiceSelection & 1 ) && _currentChoiceSelection < 6
                    ...
                    _partyUI->drawPartyPkmnSub( p_selectedIdx, true, false );
                    _ranges = _partyUI->drawPartyPkmnChoice( p_selectedIdx, c + 6,
        ...
        for( auto i : _ranges ) {
            if( i.first.inRange( touch ) ) {
                ...
                if( !bad ) {
                    if( i.second < 6 ) {
                        SOUND::playSoundEffect( SFX_CHOOSE );
                        res = 1 + ( executeChoice( _currentChoices[ _currentChoiceSelection ] )
                                && _allowCancel );
