...
#define ANCHOR_Y 8
    ...
    void boxUI::initSub( ) {
        ...
        // box arrow
        tileCnt
            = IO::loadSpriteB( SPR_SEL_ARROW_OAM_SUB, tileCnt, 0, 0, 16, 16,
                               box_arrowPal, box_arrowTiles,
                               box_arrowTilesLen, false, false, true, OBJPRIORITY_0, true );

        // Party bg
        IO::loadSpriteB( SPR_PARTY_BG_OAM_SUB + 1, tileCnt,
                         80, 192 - 32, 32, 32, 0, 0, 0, true, true, false, OBJPRIORITY_2, true );
        IO::loadSpriteB( SPR_PARTY_BG_OAM_SUB + 2, tileCnt,
                         64, 192 - 32, 32, 32, 0, 0, 0, true, true, false, OBJPRIORITY_2, true );
        tileCnt = IO::loadSpriteB( SPR_PARTY_BG_OAM_SUB, tileCnt,
                         32, 192 - 20, 32, 32, noselection_64_20Pal, noselection_64_20Tiles,
                         noselection_64_20TilesLen, false, false, false, OBJPRIORITY_2, true );

        std::memset( TEXT_BUF, 0, sizeof( TEXT_BUF ) );
        IO::regularFont->printStringBC( GET_STRING( 380 ),
                                        BG_PAL( true ), TEXT_BUF, 64, IO::font::CENTER );
        tileCnt = IO::loadSpriteB( SPR_PARTY_TEXT_OAM_SUB, tileCnt, 40, 192 - 18, 64, 32,
                                   TEXT_BUF, 64 * 32 / 2, false, false, false, OBJPRIORITY_1, true );
        std::memset( TEXT_BUF, 0, sizeof( TEXT_BUF ) );
        IO::regularFont->printStringBC( GET_STRING( 381 ),
                                        BG_PAL( true ), TEXT_BUF, 64, IO::font::CENTER );
            ...
            if( p_box->m_pokemon[ i ].getSpecies( ) ) {
                if( !p_box->m_pokemon[ i ].isEgg( ) ) {
                    ...
                    IO::loadEggIconB( oam[ SPR_PKMN_START_OAM_SUB + i ].x,
                                      ...
                                      oam[ SPR_PKMN_START_OAM_SUB + i ].gfxIndex, true,
                                      p_box->m_pokemon[ i ].getSpecies( ) == PKMN_MANAPHY );
                }
            } else {
                oam[ SPR_PKMN_START_OAM_SUB + i ].isHidden = true;
            }
            ...
            case BUTTON_PKMN_TAKE_ITEM:
                ...
                IO::Oam->oamBuffer[ SPR_SEL_ARROW_OAM_SUB ].x =
                    IO::Oam->oamBuffer[ SPR_PKMN_OPTS_OAM_SUB(
                            p_selectedButton - BUTTON_PKMN_MOVE ) ].x;
                IO::Oam->oamBuffer[ SPR_SEL_ARROW_OAM_SUB ].y =
                    IO::Oam->oamBuffer[ SPR_PKMN_OPTS_OAM_SUB(
    ...
    void boxUI::drawPkmnInfoTop( pokemon* p_pokemon ) {
        ...
        tileCnt
            = IO::loadSprite( SPR_NAME_BG_OAM + 3, SPR_BOX_PAL,
                               tileCnt, 0, -4, 64, 32, noselection_128_32_2Pal,
                               noselection_128_32_2Tiles, noselection_128_32_2TilesLen, false,
        ...
        IO::updateOAM( false );
        if( !p_pokemon || !p_pokemon->getSpecies( ) ) {
            return;
        }

        // Name box
        IO::loadSprite( SPR_CHOICE_START_OAM, SPR_BOX_PAL, tileCnt, ANCHOR_X, ANCHOR_Y,
                        16, 32, 0, 0,
                        noselection_96_32_1TilesLen, false, false, false, OBJPRIORITY_3, false,
                        OBJMODE_BLENDED );
        ...
        IO::loadSprite( SPR_CHOICE_START_OAM + 1, SPR_BOX_PAL, tileCnt, ANCHOR_X + 16,
                ANCHOR_Y, 16, 32, 0, 0,
                        noselection_96_32_2TilesLen, false, false, false, OBJPRIORITY_3, false,
                        OBJMODE_BLENDED );
        IO::loadSprite( SPR_CHOICE_START_OAM + 2, SPR_BOX_PAL, tileCnt, ANCHOR_X + 32,
                ANCHOR_Y, 16, 32, 0, 0,
                        noselection_96_32_2TilesLen, false, false, false, OBJPRIORITY_3, false,
                        OBJMODE_BLENDED );
        IO::loadSprite( SPR_CHOICE_START_OAM + 3, SPR_BOX_PAL, tileCnt, ANCHOR_X + 48,
                ANCHOR_Y, 16, 32, 0, 0,
                        noselection_96_32_2TilesLen, false, false, false, OBJPRIORITY_3, false,
                        OBJMODE_BLENDED );
        IO::loadSprite( SPR_CHOICE_START_OAM + 7, SPR_BOX_PAL, tileCnt, ANCHOR_X + 73,
                ANCHOR_Y, 16, 32, 0, 0,
                        noselection_96_32_2TilesLen, false, false, false, OBJPRIORITY_3, false,
                        OBJMODE_BLENDED );
        ...
        IO::loadSprite( SPR_MOVEWINDOW_START_OAM + 3, SPR_INFOPAGE_PAL, tileCnt, ANCHOR_X - 6,
                        ANCHOR_Y + 119, 64, 64,
                        0, 0, infopage1TilesLen, false, false, p_pokemon->isEgg( ),
                        OBJPRIORITY_3, false, OBJMODE_BLENDED );
        IO::loadSprite( SPR_MOVEWINDOW_START_OAM + 1, SPR_INFOPAGE_PAL, tileCnt, ANCHOR_X + 40,
                        ANCHOR_Y + 119 + 64 - 52, 64, 64,
                        0, 0, infopage1TilesLen, true, true, p_pokemon->isEgg( ),
                        OBJPRIORITY_3, false, OBJMODE_BLENDED );
        ...
        IO::loadSprite( SPR_MOVEWINDOW_START_OAM + 2, SPR_INFOPAGE_PAL, tileCnt, ANCHOR_X + 40,
                        ANCHOR_Y + 119,
                        64, 64, 0, 0, infopage2TilesLen, false, false, p_pokemon->isEgg( ),
                        OBJPRIORITY_3, false, OBJMODE_BLENDED );
        IO::loadSprite( SPR_MOVEWINDOW_START_OAM, SPR_INFOPAGE_PAL, tileCnt, ANCHOR_X - 6,
                        ANCHOR_Y + 119 + 64 - 52,
                        64, 64, 0, 0, infopage2TilesLen, true, true, p_pokemon->isEgg( ),
                        OBJPRIORITY_3, false, OBJMODE_BLENDED );
        ...
        if( !p_pokemon->isEgg( ) ) {
            ...
            for( u8 i = 0; i < 2; ++i ) {
                tileCnt
                    = IO::loadTypeIcon( data.m_baseForme.m_types[ i ]
                            , INFO_X + 128 - 62 + 34,
                            INFO_Y + 11 + 15 * i, SPR_TYPE_OAM( i ), SPR_TYPE_PAL( i ),
                            tileCnt, false, CURRENT_LANGUAGE );
            ...
            // Shiny
            tileCnt = IO::loadSprite( SPR_SHINY_ICON_OAM, SPR_SHINY_ICON_PAL, tileCnt, ANCHOR_X +
                    68 - 8, ANCHOR_Y + 20, 8, 8, status_shinyPal, status_shinyTiles,
                    status_shinyTilesLen, false, false,
                    !p_pokemon->isShiny( ), OBJPRIORITY_0,
                    false, OBJMODE_NORMAL );

            // Pokéball Icon
            tileCnt = IO::loadItemIcon( ITEM::ballToItem( p_pokemon->m_boxdata.m_ball ),
                    ANCHOR_X - 4, ANCHOR_Y,
                    SPR_BALL_ICON_OAM, SPR_BALL_ICON_PAL, tileCnt, false );
            ...
            IO::regularFont->printStringC( buffer,
                    IO::regularFont->stringWidthC( GET_STRING( 337 ) )
                    + INFO_X + 16, INFO_Y + 27, false );
            IO::regularFont->setColor( IO::BLACK_IDX, 1 );
            IO::regularFont->setColor( 0, 2 );
            ...
            IO::regularFont->printStringC( GET_STRING( 187 + u8( p_pokemon->getNature( ) ) ),
                    INFO_X + 144 - 12, INFO_Y + 27 + 30, false, IO::font::RIGHT );
            IO::regularFont->printStringC( getAbilityName( p_pokemon->getAbility( ) ).c_str( ),
                    INFO_X + 144 - 12, INFO_Y + 27 + 60, false, IO::font::RIGHT );
            ...
            if( p_pokemon->getItem( ) ) {
                ...
                IO::regularFont->printStringC( ITEM::getItemName( p_pokemon->getItem( ) ).c_str( ),
                        INFO_X + 144 - 12, INFO_Y + 27 + 90, false, IO::font::RIGHT );
            }

            // Moves
            for( u8 i = 0; i < 4; ++i ) {
                if( p_pokemon->m_boxdata.m_moves[ i ] ) {
                    IO::regularFont->printStringC( MOVE::getMoveName(
                            p_pokemon->m_boxdata.m_moves[ i ] ).c_str( ), ANCHOR_X + 2, ANCHOR_Y +
                            123 + 15 * i, false );
        ...
        // pokemon / shadow
        if( !p_pokemon->isEgg( ) ) {
            tileCnt = IO::loadPKMNSprite( p_pokemon->getSpecies( ), ANCHOR_X,
                    ANCHOR_Y + 26, SPR_PKMN_START_OAM,
                    SPR_PKMN_PAL, tileCnt, false, p_pokemon->isShiny( ),
                    p_pokemon->isFemale( ), false, false,
                    p_pokemon->getForme( ) );
        } else {
            tileCnt = IO::loadEggSprite( ANCHOR_X, ANCHOR_Y + 26,
        ...
        IO::loadSprite( SPR_PKMN_SHADOW_START_OAM + 0, SPR_PKMN_SHADOW_PAL,
                oam[ SPR_PKMN_START_OAM + 0 ].gfxIndex, ANCHOR_X - 48 - sx, ANCHOR_Y + 26 - sy,
                64, 64,
