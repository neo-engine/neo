...
#define MAX_PKMN_PER_PAGE( p_mode ) \
    ( ( p_mode == dex::SHOW_CAUGHT ) ? MAX_PKMN_CAUGHT : MAX_PKMN_ALL )
...
#define DRAW_TOP( ) _dexUI->drawPage( CURR_PKMN, _page, _forme )
    ...
    void dex::changeMode( mode p_newMode, u16 p_startPkmn ) {
        ...
        for( u8 i = 0; i < MAX_PKMN_PER_PAGE( p_newMode ); ++i ) {
            if( p_newMode == SHOW_SINGLE && i ) break;
    ...
    void dex::rotateForward( ) {
        if( _mode == SHOW_ALL ) {
            ...
            for( u8 i = 0; i < 8; ++i ) _curPkmn[ ( _curPkmnStart + i ) % MAX_PKMN_ALL ] = ++end;
            _curPkmnStart = ( _curPkmnStart + 8 ) % MAX_PKMN_ALL;
        } else if( _mode == SHOW_CAUGHT ) {
            _curPkmn[ _curPkmnStart ] = nextEntry(
        ...
        loop( ) {
            ...
            if( _mode == SHOW_CAUGHT ) {
                ...
                if( GET_AND_WAIT( KEY_DOWN ) )
                    ...
                    select( ( _selectedIdx + 2 * ( 4 - ( ( _selectedIdx + 2 ) % 4 ) ) - 1 )
                            % MAX_PKMN_ALL );
                else if( GET_AND_WAIT( KEY_LEFT ) )
                    select( ( _selectedIdx + MAX_PKMN_ALL - 2 * ( ( _selectedIdx + 2 ) % 4 ) - 1 )
                            % MAX_PKMN_ALL );
