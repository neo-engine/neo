...
devoptab_t nitroFSdevoptab
    = { "nitro",
        ...
        &nitroFSOpen,  //	int (*open_r)(struct _reent *r, void *fileStruct, const char *path,int
                       // flags,int mode);
        ...
        &nitroFSDirOpen,  //	DIR_ITER* (*diropen_r)(struct _reent *r, DIR_ITER *dirState, const char
                          //*path);
        &nitroDirReset,   //	int (*dirreset_r)(struct _reent *r, DIR_ITER *dirState);
        &nitroFSDirNext,  //	int (*dirnext_r)(struct _reent *r, DIR_ITER *dirState, char *filename,
        ...
        if( strncmp( __system_argv->argv[ 0 ], "fat", 3 ) == 0
            ...
            if( fatInitDefault( ) ) {
                ...
                if( ndsFileFD != -1 ) {
                    ...
                    if( nitropath != NULL ) { nitropath = getcwd( nitropath, PATH_MAX ); }
                }
            }
        }
    }
...
static void nitroSubReadBlock( u32 pos, u8 *ptr, u32 len ) {
    ...
    if( ( len & 3 ) == 0 && ( ( (u32) ptr ) & 3 ) == 0 && ( pos & 3 ) == 0 ) {
        ...
        static u32 temp[ 128 ];
        cardParamCommand( 0xB7, ( pos & ~0x1ff ),
                          CARD_DELAY1( 0x1FFF ) | CARD_DELAY2( 0x3F ) | CARD_CLK_SLOW | CARD_nRESET
                              | CARD_SEC_CMD | CARD_SEC_DAT | CARD_ACTIVATE | CARD_BLK_SIZE( 1 ),
                          temp, 0x200 >> 2 );
...
static inline int nitroSubRead( unsigned int *npos, void *ptr, u32 len ) {
    ...
    if( cardRead ) {
        ...
        if( ndsFileLastpos != *npos ) lseek( ndsFileFD, *npos, SEEK_SET );
...
static int nitroFSDirNext( struct _reent *, DIR_ITER *dirState, char *filename, struct stat *st ) {
    ...
    if( next ) {
        if( next & NITROISDIR ) {
            ...
            if( st )
                st->st_size = dirStruct->romfat.bottom - dirStruct->romfat.top; // calculate
                                                                                // filesize
        }
...
static int nitroFSOpen( struct _reent *, void *fileStruct, const char *path, int, int ) {
    ...
    do {
        if( ( *cptr == '/' )
            ...
            cptr++;
            strncpy( dirfilename, path,
                     cptr - path ); // copy string up till and including/ or : zero rest
...
static int nitroFSRead( struct _reent *, void *fd, char *ptr, size_t len ) {
    ...
    if( *npos + len > fatStruct->end ) len = fatStruct->end - *npos; // dont read past the end
    if( *npos > fatStruct->end ) return ( 0 );                       // hit eof
...
static off_t nitroFSSeek( struct _reent *, void *fd, off_t pos, int dir ) {
    ...
    if( pos > fatStruct->end ) return ( -1 ); // dont read past the end
